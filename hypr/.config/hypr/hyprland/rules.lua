...
hl.window_rule({
  name = "float-small",
  match = { title = [[^(Open Folder|Open File|Save File|Save Folder|Save Image|Save As|Open As|Extension: \(Bitwarden Password Manager\))$]] },
  float = true,
  size = "50% 50%",
  center = true,
})
...
hl.window_rule({ match = { class = "^(xfce-polkit|wleave|nwg-look|nm-connection-editor|org.twosheds.iwgtk|overskride|satty)$" }, float = true })
...
hl.window_rule({ match = { title = "^(.*YouTube.*|Picture-in-Picture|.*Mozilla.*|.*localhost.*|.*Edge.*|.*Cursor.*|.*Grayjay.*)$" }, opacity = "1.0 override 1.0 override" })
