...
return {
	...
	dependencies = {
		...
		-- DAP UI
		{
			...
			opts = {}, -- Optional: Add dap-ui options here if needed
			config = function(_, opts)
				local dapui = require("dapui")
				dapui.setup(opts) -- Setup dap-ui
	...
	config = function()
		...
		-- REMOVE the hardcoded java configurations - let jdtls handle this
		-- dap.configurations.java = { ... } -- DELETE THIS BLOCK
		...
		dap.adapters["pwa-node"] = {
			...
			executable = {
				...
				command = vim.fn.stdpath("data") .. "/mason/packages/js-debug-adapter/js-debug/src/dapDebugServer.js",
				args = { "${port}" },
			},
		}
		...
		-- If fallback doesn't work for node, explicitly set it for the adapter:
		-- dap.adapters['pwa-node'].external_terminal = { ... }
		...
		print("nvim-dap configuration loaded.")
	end,
